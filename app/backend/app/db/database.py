    ...
    with open("megekko_products-test.json") as f:
        ...
        for category in data:
            for i in range(0, 10):
                ...
                #     session.add(CPU(sku=product["sku"], socket=specs["Socket"], cores=specs["Processor Cores"], threads=specs["Processor aantal threads"], base_clock=float(specs["Processor Snelheid"].split(" ")[0]), boost_clock=boost_clock, power_consumption=specs["TDP (max)"]))
